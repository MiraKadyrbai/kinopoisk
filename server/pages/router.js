...
router.get('/', async (req, res) =>{
  ...
  if (req.query.search && req.query.search.length > 0) {
    ...
    const films = await Film.find(options).limit(limit).skip(page * limit).populate('country', 'name').populate('genre')
...
router.get('/new', async(req, res)=>{
    ...
    res.render("newFilm", {genres : allGenres, counties: allCountries, user: req.user ? req.user:{}})
})
...
router.get('/edit/:id', async(req, res)=>{
    ...
    res.render("editFilm", {genres : allGenres, counties: allCountries, user: req.user ? req.user:{}, film})
})
